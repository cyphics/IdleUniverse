...
describe('RateComputer getProductionRate', () => {
  it('empty rate', () => {
  ...
  it('buy first upgrade', () => {
    ...
    const rate = manager.computer.getProductionRate(resourceId.joule);
  ...
  it('test resource not upgraded', () => {
    const manager = new UpgradeManager();
    manager.buyUpgrade(upgradesId.i_lvl_1_coil);
    const rate = manager.computer.getProductionRate(resourceId.dark_matter);
    expect(rate).to.be.eql(0);
  });
});
