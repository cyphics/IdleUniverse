...
describe('Price getResources', () => {
  it('get empty resource', () => {
    const price = new Price([new ResourceAmount(resourceId.joule, 10)]);
    const resourceAmount = price.getAmount(resourceId.dark_matter);
    expect(resourceAmount).to.be.eql(0);
  });
  it('set 1 resource', () => {
    const price = new Price([new ResourceAmount(resourceId.joule, 10)]);
    const resourceAmount = price.getAmount(resourceId.joule);
    expect(resourceAmount).to.be.eql(10);
  });
  it('set 2 resources', () => {
    const price = new Price([
      new ResourceAmount(resourceId.joule, 10),
...
describe('Price addResource', () => {
  it('add empty resource', () => {
    const price = new Price();
    price.addResource(new ResourceAmount(resourceId.joule, 0));
    const finalAmount = price.getAmount(resourceId.joule);
    expect(finalAmount).to.be.eql(0);
  });
  it('add new resource', () => {
    const price = new Price();
    price.addResource(new ResourceAmount(resourceId.joule, 1));
    const finalAmount = price.getAmount(resourceId.joule);
...
describe('Price multiply', () => {
  it('factor 1 (no change)', () => {
    const price = new Price();
    price.addResource(new ResourceAmount(resourceId.joule, 10));
    price.multiply(1);
    const kinAmount = price.getAmount(resourceId.joule);
  ...
  it('factor 2 ', () => {
    ...
    const kinAmount = newPrice.getAmount(resourceId.joule);
